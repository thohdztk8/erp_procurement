...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiCashRegister" title="Kế toán & Thanh toán" main>
      <BaseButton v-if="activeTab === 'notes' && isAccountant" :icon="mdiPlus" label="Thêm Credit Note" color="contrast"
        small @click="showCreditModal = true" />
      <BaseButton v-if="activeTab === 'notes' && isAccountant" :icon="mdiPlus" label="Thêm Debit Note" color="info"
        small @click="showDebitModal = true" />
      <BaseButton v-if="isAccountant" :icon="mdiFileExport" label="Xuất MISA/FAST" color="warning" small
    ...
    <div class="flex space-x-4 mb-4 border-b">
      <button class="pb-2 px-4 text-sm font-semibold transition"
        ...
        @click="activeTab = 'invoices'">Hóa đơn NCC ({{ invoiceTotalItems }})</button>
      <button class="pb-2 px-4 text-sm font-semibold transition"
        :class="activeTab === 'payments' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-800'"
        @click="activeTab = 'payments'">Yêu cầu chi ({{ paymentTotalItems }})</button>
      <button class="pb-2 px-4 text-sm font-semibold transition"
        :class="activeTab === 'notes' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-800'"
        @click="activeTab = 'notes'">Credit/Debit Notes</button>
      <button class="pb-2 px-4 text-sm font-semibold transition"
        :class="activeTab === 'reports' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-800'"
        @click="activeTab = 'reports'">Thống kê & KPI</button>
    </div>
    ...
    <CardBox v-if="activeTab === 'invoices'" has-table>
      <table class="w-full text-xs text-left">
        ...
        <tbody>
          <tr v-for="inv in invoices" :key="inv.invoice_id" class="border-b">
            ...
            <td class="px-6 py-3"><span class="rounded px-2 py-0.5 text-[10px] font-bold"
                :class="inv.matching_status === 'MATCHED' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">{{
                  inv.matching_status || 'CHƯA ĐỐI CHIẾU' }}</span></td>
            <td class="px-6 py-3 text-right">
              <BaseButtons type="justify-end" no-wrap>
                <BaseButton color="info" :icon="mdiRefresh" label="Đối chiếu" small
                  @click="handleVerify(inv.invoice_id)" />
                <BaseButton v-if="inv.matching_status === 'MISMATCHED'" color="danger" :icon="mdiAlertDecagram"
                  label="Ghi đè" small @click="handleOverride(inv.invoice_id)" />
                <BaseButton v-if="inv.matching_status === 'MATCHED' || inv.matching_status === 'OVERRIDDEN'"
                  color="success" :icon="mdiCheckDecagram" label="Yêu cầu chi" small
      ...
      <BasePagination :current-page="invoicePage" :total-pages="invoiceTotalPages" :total-items="invoiceTotalItems"
          ...
          <tr v-for="p in payments" :key="p.payment_req_id" class="border-b">
            ...
            <td class="px-6 py-3 text-right">
              <BaseButtons v-if="p.req_status === 'PENDING'" type="justify-end" no-wrap>
                <BaseButton color="success" label="Duyệt" small
                  @click="handleProcessPayment(p.payment_req_id, 'APPROVE')" />
                <BaseButton color="danger" label="Từ chối" small
                  @click="handleProcessPayment(p.payment_req_id, 'REJECT')" />
              </BaseButtons>
            </td>
          </tr>
        </tbody>
      </table>
      <BasePagination :current-page="paymentPage" :total-pages="paymentTotalPages" :total-items="paymentTotalItems"
      ...
      <table class="w-full text-xs text-left">
        ...
        <tbody>
          <tr v-for="n in [...creditNotes, ...debitNotes]" :key="n.credit_note_id || n.debit_note_id" class="border-b">
            ...
            <td class="px-6 py-3 font-bold" :class="n.credit_amount ? 'text-green-600' : 'text-orange-600'">{{
              formatCurrency(n.credit_amount || n.debit_amount) }}</td>
    ...
    <div v-else-if="activeTab === 'reports'" class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <CardBox class="p-6 text-center">
        <h4 class="text-sm font-bold text-gray-500 mb-2">Đơn mua hàng dở dang</h4>
        <p class="text-3xl font-black text-blue-600">{{ reportsData.kpi?.ipos_in_progress || 0 }}</p>
      </CardBox>
      <CardBox class="p-6 text-center">
        <h4 class="text-sm font-bold text-gray-500 mb-2">Thanh toán quá hạn</h4>
        <p class="text-3xl font-black text-red-600">{{ reportsData.kpi?.overdue_payments || 0 }}</p>
      </CardBox>
      <CardBox class="p-6 text-center">
        <h4 class="text-sm font-bold text-gray-500 mb-2">Vật tư tồn kho thấp</h4>
        <p class="text-3xl font-black text-yellow-600">{{ reportsData.kpi?.low_stock_items || 0 }}</p>
      </CardBox>
    </div>
  </SectionMain>
